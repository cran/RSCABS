...
\par
	...
	RSCABS is a step down anylsis so, if the test statistic is calculated to be significant (p-value $\geq$  0.05) then 
...
\item Rao, J. N. K. and Scott, A. J. A. 1992. Simple Method for the Analysis of Clustered Data. \textit{Biometrics}, \textbf{48}: 577\textemdash586. 
